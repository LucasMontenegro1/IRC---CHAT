...
impl DccSendMessage {
    pub fn new(msg: &DccMessage) -> Result<Self, ErrorServer> {
        ...
        let size = Self::get_size(msg.clone())?;
        let hash = Self::get_hash(msg.clone())?;
    ...
    fn get_size(msg: DccMessage) -> Result<String, ErrorServer> {
        let params = msg.parameters();
        match params {
            Some(parameters) => match parameters.get(3) {
                None => Ok(String::new()),
                Some(str) => Ok(str.to_owned()),
            },
            None => Ok(String::new()),
    ...
    fn get_hash(msg: DccMessage) -> Result<String, ErrorServer> {
        let params = msg.parameters();
        match params {
            Some(parameters) => match parameters.get(4) {
                None => Ok(String::new()),
                Some(str) => Ok(str.to_owned()),
            },
            None => Ok(String::new()),
    ...
    fn send_message_to(
        ...
        let _ = channel;
        let chat = DccSend::new(self.msg.clone())?;
    ...
    pub fn format_message(&self, msg: DccSend) -> String {
        let _ = msg;
        format!(
            ":{} PRIVMSG {} dcc send {} {} {} {} {}",
            self.from, self.to, self.filename, self.ip, self.port, self.size, self.hash
        )
    }
}
