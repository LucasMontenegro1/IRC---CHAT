...
impl DccAcceptMessage {
    pub fn new(msg: &DccMessage) -> Result<Self, ErrorServer> {
        ...
        let offset = Self::get_offset(msg.clone())?;
    ...
    fn get_offset(msg: DccMessage) -> Result<String, ErrorServer> {
        let params = msg.parameters();
        match params {
    ...
    fn send_message_to(
        ...
        let _ = channel;
        let chat = DccAccept::new(self.msg.clone())?;
    ...
    pub fn format_message(&self, msg: DccAccept) -> String {
        let _ = msg;
        format!(
            ":{} PRIVMSG {} dcc accept {} {} {} {} ",
            self.from, self.to, self.filename, self.ip, self.port, self.offset
        )
    }
}
