...
impl DccChatMessage {
    pub fn new(msg: &DccMessage) -> Result<Self, ErrorServer> {
        ...
        let protocol = Self::get_protocol(msg.clone())?;
    ...
    fn get_protocol(msg: DccMessage) -> Result<String, ErrorServer> {
            ...
            Some(mut c) => match channel {
                None => {
                    ...
                    if c.see_if_clonable().is_some() {
                        println!("DCC CHAT - Tries to send messages to {}", c.get_nickname());
                        self.send_message_to(&mut c, channel, away)?
                    } else {
                        println!("DCC CHAT - Nothing to do");
                        Reply::rpl_none()
                    }
                }
    ...
    fn send_message_to(
        ...
        let _ = channel;
        let chat = DccChat::new(self.msg.clone())?;
    ...
    pub fn format_message(&self, msg: DccChat) -> String {
        format!(
