...
impl ContactsWindow {
    pub fn new() -> Self {
        ...
        menu_bar.append(Some("Who Is?"), Some("app.whois"));

        let popover = gtk::PopoverMenu::builder().build();
        ContactsWindow {
            list_store_channels: _list_store_channels,
            list_store_contacts: _list_store_contacts,
            menu: menu_bar,
            invite_menu: _invite_menu,
            contact: String::new(),
            channel: String::new(),
            menu_model: popover,
            app: None,
    ...
    pub fn set_channel(&mut self, channel: String) {
        println!("SETEANDO EL CANAL EN CONTACTS WINDOW COMO {}", channel);
...
impl Default for ContactsWindow {
    fn default() -> Self {
        Self::new()
    }
}
